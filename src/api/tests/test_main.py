...
def test_get_user_informations(client, test_user):
    ...
    response = client.get(
        "/get_user_informations", headers={"Authorization": f"Bearer {token}"}
    )
    assert (
        response.status_code == 200
    ), f"Unexpected status code: {response.status_code}"
    ...
    assert (
        "favorite_model" in response_json
    ), "Response JSON does not contain 'favorite_model'."
    assert isinstance(
        response_json["favorite_model"], int
    ), "'favorite_model' is not an integer."
...
def test_colorize_bw_image(client, test_user):
    token = test_auth_token(client, test_user)
    response = client.get(
        "/colorize_bw_image", headers={"Authorization": f"Bearer {token}"}
    )
    assert (
        response.status_code == 200
    ), f"Unexpected status code: {response.status_code}"
...
def test_colorized_images_list_download_rate_image(client, test_user):
    token = test_auth_token(client, test_user)
    response = client.get(
        "/get_colorized_images_list", headers={"Authorization": f"Bearer {token}"}
    )
    assert (
        response.status_code == 200
    ), f"Unexpected status code: {response.status_code}"
    response2 = client.get(
        f"/download_colorized_image/{list(response.json().keys())[0]}",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert (
    ...
    response3 = client.post(
        f"/rate_colorized_image/{list(response.json().keys())[0]}?rating=5",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert (
    ...
    assert response3.json() == {
        "message": "Your colorized image has been successfully rated !"
    }
...
def test_download_last_colorized_image(client, test_user):
    token = test_auth_token(client, test_user)
    response = client.get(
        "/download_last_colorized_image", headers={"Authorization": f"Bearer {token}"}
    )
    assert (
        response.status_code == 200
    ), f"Unexpected status code: {response.status_code}"
...
def test_set_favorite_model(client, test_user):
    ...
    assert (
        response3.status_code == 200
    ), f"Unexpected status code: {response3.status_code}"
...
def test_users_list(client, test_admin):
    ...
    response = client.get(
        "/get_users_list", headers={"Authorization": f"Bearer {token}"}
    )
    assert (
        response.status_code == 200
    ), f"Unexpected status code: {response.status_code}"
...
def test_disable_user(client, test_admin):
    token = test_auth_token(client, test_admin)
    response = client.post(
        "/disable_user?username=unittestuser",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert (
        response.status_code == 200
...
def test_enable_user(client, test_admin):
    token = test_auth_token(client, test_admin)
    response = client.post(
        "/enable_user?username=unittestuser",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert (
        response.status_code == 200
...
def test_set_admin_access(client, test_admin):
    token = test_auth_token(client, test_admin)
    response = client.post(
        "/set_admin_access?username=unittestuser",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert (
        response.status_code == 200
...
def test_delete_user(client, test_user):
    token = test_auth_token(client, test_user)
    response = client.post(
        "/delete_user?username=unittestuser",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert (
        response.status_code == 200
    ...
    assert response.json() == {
        "message": "User(unittestuser) + Data deleted successfully !"
    }
